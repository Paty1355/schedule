...
from schedule_algorithm.data_model.course import Course_assignments
from schedule_algorithm.data_model.location import Rooms
from schedule_algorithm.data_model.time_models import Week_parity, Time_slots, Weekdays
from schedule_algorithm.genetic_algorithms_tool.algorithm_data_schema import Gene
...
class ScheduleTools:
    ...
    def time_slots_generator(self, curse_time_per_week, idx):
        start_time_course= (random.randint(self.start_time_course_in_minute,self.end_time_course_in_minute))
        ...
        for i, course in enumerate(course_assignments,start=1):
            ...
            genotype.append(
                Gene(
                    ...
                    time_slot_id = self.time_slots_generator(hours/(15 if week_parity_value.value == "both" else 7), i)
                )
            )
        return genotype
